...
def stokes_solver(w, func, n = 50 ):
	...
	# burde endre hva func er for hver gang. Sa lage en def som gir en ny func for her K
	K   = interpolate(func, Pspace)  # Kontrollen i modellen 
	...
	# Applying Dirichlet type conditions to part of the domain______________________________________
...
def WSS_Solver(w,func, n):
	U,P = stokes_solver(w,func,n)	
	shear_stress = project(0.5*(U[0].dx(1) + U[1].dx(0)), ShearStressSpace) 
	plot(shear_stress, interactive = True, title = 'WSS')	                
	WSSarray = shear_stress.vector().array()                                
       	#len(WSSarray) 							# 15 000, it means WSS is calculated in 15 000 DC nodes
	...
	print 'smallest WSS value',smallest, 'largest WWS value', largest 
